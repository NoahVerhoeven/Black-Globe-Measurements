...
We note that in our project, we are dealing with forced convection, as such a good formula for the convective heat transfer coefficient $h$ is
$$
h = 6.7\frac{V_a^{0.6}}{D^{0.4}}
$$
which is valid in standard conditions. For higher wind speeds, the rate of change of the heat of the globe will be high, and we reach equilibrium much faster. \textbf{It is very important to emphasize that the convective heat transfer coefficient doesn't the true MRT, only our estimate, since the heat ex}
\section{Recreating the true MRT function}

From our simulations we see that direct estimation with our equation \eqref{eq:ideal-grey} isn't accurate for a non-static environment, thus meaning a changing true MRT function. In order to allow accurate measurements of the true MRT, our estimates need to be manipulated. In the context of this manuscript, we will refer to this process as 'recreating' the true MRT function based on the provided estimates. In general, we seek a transformation $\divideontimes$ such that
$$
 \divideontimes: \: MRT_{\text{Emp}}(t) \rightarrow MRT(t)
$$
With $MRT_{\text{Emp}}(t)$ the empirically measured-, and $MRT(t)$ the true MRT. We can research this by using our simulated estimates as the input in $\divideontimes$, we can then quantitatively explore how well this recreated true MRT matches the true (synthetic) MRT.

\subsection{Collocation Method}
Measurements and estimates are unique defined by the true MRT function. If we assume the through MRT has some form, e.g. a linear combination of basis functions, we might be able to numerically fit the coefficients of this form to best reproduce the measured data through simulation. So we estimate the coefficients of the true MRT function, we simulate our estimate using these coefficients, and we compare with the actual data. If the simulation matches, we have a good fit, otherwise we try finding better coefficients. My current though is the use either an $n$-order polynomial (so $n$ coefficients) or a fourier series.

\subsection{Moving Average}
From experimenting with the simulation we found that a moving average of the true MRT with an optimal window can recreate our estimate $MRT_{\text{est}}$ very nicely. In figure \ref{fig:moving-average-1} we see that this moving average over the true MRT function indeed recreates the estimated MRT quite well.
\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\textwidth]{Moving-average-1}
	\caption{Moving average over the true MRT function, we find that it matches the estimated MRT quite well. We numerically found the optimal window size which resulted in the smallest absolute error between the moving average and the estimated MRT.}
	\label{fig:moving-average-1}
\end{figure}
The moving average approach can be generalized to an arbitrary moving average matrix $\mathbf{A}$. We therefore find that
$$
\mathbf{A} T_{\text{MRT}} \approx T_{\text{est}}
$$
With $T_{\text{est}}$ the estimated MRT, which would correspond to our empirical data. The matrix $\mathbf{A}$ has the property that all rows, when summed, must equal one. Within these constraints, it can have an arbitrary for. In the figure \ref{fig:moving-average-1} we used an equally weighted moving average, meaning that all non-zero terms had the same values (which is based on the window size). We found that optimal window size for this simulation to be 91, although this depends heavily on the amount of data points, here we used 1000 simulated data points.
\\

If a moving average matrix $\mathbf{A}$ proves to effectively recreate the estimated MRT, we can simply apply the inverse $\mathbf{A}^{-1}$ to find
$$
T_{\text{MRT}} = \mathbf{A}^{-1}T_{\text{est}}
$$
We again provide a concrete example of this figure \ref{fig:moving-average-2}.
\begin{figure}[H]
	\centering
	\includegraphics[width=0.90\textwidth]{Moving-average-2}
	\caption{Recovered true MRT based on the inverse moving average matrix. We used a constant (equally weighted) moving average with an optimal window size of 901, this was found based on 10000 simulated data points.}
	\label{fig:moving-average-2}
\end{figure}
We find that this method, when using a constant moving average, more efficiently reproduces the true MRT in sections of high variability. Notably, between minutes 7.5 and 12.5, the recovered true MRT is a better match than the normal estimate. However, in regions were the moving average doesn't differ much from the estimate, the recovered true MRT is a worse alternative. For these sections, we find that the estimated MRT doesn't differ much from the true MRT. Nonetheless, we find that this is a method worth investigating.
\\

We could theorize that a moving average, more weighted around the current point, better represent the overall movement of the true MRT and therefore the estimated MRT. After all, if at some the true MRT is rapidly changing, the estimated MRT will also be rapidly changing. Be weighting to the right, or the current point, we takes these effects better into account. Indeed, if we use a linearly decay (meaning that as we get further from the current point, the weights linearly decrease) moving average, the moving average better reproduces the estimated MRT, and thus this method better recovers the true MRT, see figure \ref{fig:moving-average-3}
\begin{figure}[H]
	\centering
	\includegraphics[width=0.90\textwidth]{Moving-average-3}
	\caption{Recovered true MRT based on the inverse moving average matrix. We used a linear decay (right weighted) moving average with an optimal window size of 137, this was found based on 1000 simulated data points. The mean square error is 58.752.}
	\label{fig:moving-average-3}
\end{figure}
The linear growth scheme is derived by making sure the difference between each of the subsequent non-zero elements is the same, and of course our previous condition that summing these result in 1. Mathematically, we start from by considering the set
$$
\vec{X} = \left\{0,1,2,\ldots,x_N\right\}
$$
Since we want to weight linearly, we look at the sum
$$
\sum_{n=0}^{N}x_n = \frac{N\left(N+1\right)}{2}
$$
Therefore, we find that the set of weights $W$ abide by
$$
\frac{2}{N(N+1)}\sum_{n=0}^{N}e^{-n} = 1\quad \Longrightarrow \quad \vec{W} =\frac{2}{N(N+1)}\left\{x_0, x_1, \ldots, x_N\right\}
$$
This formula can be used to construct a linearly decaying (or growing) moving average matrix with window size $N$.
\\

We further explore this idea by making an exponential decay version, which will be even more weighted to the current point. We do this by taking $\vec{X} = \left\{0,1,2,\ldots,x_N\right\}$ and than find the weights based on $e^{-\vec{X}}$, we know that
$$
\sum_{n=0}^{\infty}e^{-n} = \frac{e}{e-1}
$$
Since $0 < e^{-1} < 1$, so this is a geometric series. For a finite sum we find
$$
\sum_{n=0}^{N}e^{-n} = \frac{1 - e^{-(N+1)}}{1-\frac{1}{e}}
$$
Which indeed converges to the previous identity as $N\rightarrow\infty$. We therefore find that the exponential weights abide by
$$
\frac{1-\frac{1}{e}}{1 - e^{-(N+1)}}\sum_{n=0}^{N}e^{-n} = 1\quad \Longrightarrow \quad \vec{W} = \frac{1-\frac{1}{e}}{1 - e^{-(N+1)}}\left\{e^{-x_0}, e^{-x_1}, \ldots, e^{-x_N} \right\}
$$
Note that $e$ can be replaced with any arbitrary constant $c$, as long as the geometric series requirements holds. We find that this weighting scheme also proofs fruitful, as it can give comparable results to the linear decay method
\begin{figure}[H]
	\centering
	\includegraphics[width=0.90\textwidth]{Moving-average-4}
	\caption{Recovered true MRT based on the inverse moving average matrix. We used a exponential decay (right weighted) moving average with an optimal window size of 184, this was found based on 1000 simulated data points. The mean square error is 54.349, which is slight better than \ref{fig:moving-average-3}.}
	\label{fig:moving-average-4}
\end{figure}

Based on these results, we propose the following method to recover the true MRT from empirical estimations. We try to construct the optimal moving average matrix. We do this by choosing a method, e.g. linear. Afterwards, we try to find the optimal window size given the method and data. We can test different window sizes relative to each other by looking at how they reproduce the estimated data through this simulation. So for example, we have window sizes 100 and 110, for both these sizes we construct the true MRT. With these true MRT functions, we simulated our estimated MRT, the simulation that best matches our real empirical data is the better window size (and thus the resulting true MRT the better fit).
\\

We can test this again through simulations, we choose a true MRT and simulate our estimated MRT. We can add a little noise to this simulated estimate to mimic real empirical data. We than perform the procedure as mentioned above. The resulting estimate for the true MRT can be quantitatively compared to the synthetic true MRT.
\\

We do emphasize the fact that this is based on the assumption of continuous and smooth functions. This will in empirical data be true because of statistical noise. To recover the true MRT, we need to first smooth over our empirical data. We currently have two suggestions, \textbf{we either use a moving average again, but also keep in mind further points and we center around the current point (thus would mean that the diagonal element will always be the biggest in the row), this smooths over data but will ultimately result in noisy recovered true MRT. Alternatively, we can use spline smoothing, since this by definition results in a smooth and continuous fit to the data the recovered MRT will also be smooth and continuous.}

\subsubsection{Mathematical Justification}
Let's look at the ODE
$$
\diff{g}{t} = \frac{1}{\tau(t)}\left(u(t) - g(t)\right)
$$
We view $u(t)$ as some input signal that 'controls' were $g(t)$ goes, $\tau$ is a positive constant. If $u(t) > g(t)$, than $g(t)$ will increase (positive derivative), it will start slowing down as it approaches $u(t)$ and stop if they are equal. The constant $\tau$ determines have fast this process happens. We see that, for such systems, we can associate a vector field
$$
\mathbf{F} = \begin{pmatrix}
\tau(t) \\ g - u(t)
\end{pmatrix}\, \text{and}\, \dl{\mathbf{a}} = \begin{pmatrix}
\dl g \\ \dl t
\end{pmatrix} \quad \Longrightarrow \quad \mathbf{F} \cdot \dl{\mathbf{a}} = 0
$$
Now, if $\mathbf{F}$ is conservative, than there exist a scalarfield such $\mathbf{F} = \grad{f}$. For this to be true, we check the following identity
$$\curl{\mathbf{F}} = \begin{vmatrix}
	\mathbf{e}_1 & \mathbf{e}_2 & \mathbf{e}_3 \\
	\partial_g & \partial_t & 0 \\
	\tau(t) & g - u(t)& 0
\end{vmatrix} = \begin{pmatrix}
0 \\ 0 \\ 1 - \partial_t \tau(t)
\end{pmatrix} \neq \mathbf{0}
$$
In general, this vectorfield thus isn't conservative, we therefore find an integrating factor $\mu(g, t)$ such that it is. We find the integrating factor to be
$$
\mu(t) = e^{\int \frac{1 - \dot{\tau}(t)}{\tau(t)}\dl t }
$$
The scalarfield $f = c$ has to be a constant, we find that
$$
\partial_g f = \mu(t)\tau(t) = 0 \quad \text{and}  \quad \partial_t f = \mu(t)\left(g - u(t)\right) = 0
$$
Integrating both these differential equations leads to
$$
f = \mu(t)\tau(t)g + C_1(t) \quad \text{and} \quad f = \int \mu(t)\left(g - u(t)\right)\dl t + C_2(g)
$$
It necessarily follows that $C_2(g) = 0$, since in the first identity of $f$ no (sole) $g$-dependence is found. 
\subsubsection{Complete Algorithm}
The central assumption of this algorithm is that the true MRT is a smooth and continuous function. To preserve this quality, we must first find continuous versions of our data. To do this, we employ spline smoothing. This way the data gets smoothed out by fitting splines. This is done to all relevant time dependent variables (wind speed, air temperature and thermocouple measurements). These continuous variables, alongside other constants like globe density
\\

After we obtain continuous versions of our time dependent data, we calculate the smooth estimate of our MRT (we use the continuous variables for this to ensure a continuous MRT estimate).
\\

We, mathematically, found that the globe thermometer is a low-pass filter for the true MRT. So, conceptually, we can view the estimated MRT as a moving average of the true MRT (see mathematical justification). By inverting this moving average operation, we can recover the true MRT. To do this, we use a linearly decaying moving average matrix.
\\

The results of this moving average depend on the window size, bigger window sizes will take more of the previous points into account. Therefore, we try to find the optimal window size numerically.
\\

For each window size, we find the inverted moving average matrix and apply this to our smoothed out estimate. This results in a continuous guess for the true MRT. We use this guess, in additional with the other time dependent variables, to simulate our thermocouple measurements (see non-equilibrium states). We assign this window guess an error based on the difference between the real and simulated thermocouple measurements.
\\

If the error is small, than the underlying guess for the true MRT must decent, if the error is big, the underlying guess for the true MRT is bad. We now repeat the procedure of guessing windows until we found the window size that minimizes the error between the simulated and real thermocouple measurements.
\\

In figure \ref{fig:robust-test} we used this method, we artificially added noise to our thermocouple data. In the ideal case, we perfectly recover the true MRT (red line), however, we see that this procedure results in a close approximation of this true MRT.
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{Robust-moving-average-test}
	\caption{}
	\label{fig:robust-test}
\end{figure}

A possible issue with this method is the over- or under smoothing of the time dependent variables. Under smoothing leads to rapidly oscillating recovered MRT (this is because of the high number of oscillating splines). This can be easily caught, as it produces physically unrealistic true MRT estimates, this is of course assuming that we have prior knowledge of the environment were we take measurements. If we over smooth, these oscillations won't be present, but our recovered estimate can start to drift. Over smoothing can visually be caught by looking at the fit between the data and the spline. A good baseline for the smoothing parameter in scipy is
$$
\text{smoothing parameter} = \text{length data} \cdot \text{standard deviation of measurements}^{1.5}
$$
If non-physical oscillation is present, increase the parameter slightly until they disappear. We also advocate for slight over smoothing rather than under smoothing. This method can be used in for project, as it is robust against sudden (continuous) jumps, below we model a sudden jump in wind speed, which can occur when suddenly changing direction. The recovered true MRT shows a sudden spike in the same place, but quickly goes back to accurately following the right MRT.
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{Robust-moving-average-test-jump}
	\caption{}
	\label{fig:robust-test-jump}
\end{figure}
\textbf{The newest version of the algorithm doesn't numerically find the optimal window. Instead, we can derive (by linearizing the ODE) the smoothing parameter $\alpha$ used in the exponential smoothing. Exponential smoothing can still be seen as a moving average, expect in our algorithm the window size is necessarily the same the length of our data. So for exponential smoothing, our matrix will be a lower triangular matrix with the same properties as before. We can still take it's inverse to recover the true MRT. We do this because our globe thermometer conceptually represents a IIR filter (infinite response filter). Additionally, we now spline smooth the estimates directly (which is computationally more viable). We use the 'De wet van foutenpropagatie', see our course in statistics, to determine the standard deviations from our original measurements, we assume these to be independent. We find based on \eqref{eq:ideal-grey} that}
$$
\diffp{T_{\text{MRT}}}{T_g} = \frac{1}{4}\left(T_g^4 + \frac{6.3V_a^{0.6}}{a_g\sigma D^{0.4}}\left(T_g - T_a\right)\right)^{-\frac{3}{4}}\left(4T_g^3 + \frac{6.3V_a^{0.6}}{a_g\sigma D^{0.4}}\right)
$$
$$
\diffp{T_{\text{MRT}}}{T_a} = -\frac{6.3V_a^{0.6}}{4a_g\sigma D^{0.4}}\left(T_g^4 + \frac{6.3V_a^{0.6}}{a_g\sigma D^{0.4}}\left(T_g - T_a\right)\right)^{-\frac{3}{4}}
$$
$$
\diffp{T_{\text{MRT}}}{V_a} = \frac{1}{4}\left(T_g^4 + \frac{6.3V_a^{0.6}}{a_g\sigma D^{0.4}}\left(T_g - T_a\right)\right)^{-\frac{3}{4}}\left(\frac{0.6\cdot6.3V_a^{-0.4}}{a_g\sigma D^{0.4}}\left(T_g - T_a\right)\right)
$$
With the vector
$$
\mathbf{D} = \begin{pmatrix}
	\diffp{T_{\text{MRT}}}{T_g} \\
	\diffp{T_{\text{MRT}}}{T_a} \\
	\diffp{T_{\text{MRT}}}{V_a}
\end{pmatrix}
$$
...
\end{bmatrix}
$$
We find that the error for our estimate is
$$
\sigma_{T_{\text{MRT}}}^2 = \mathbf{D}^T\mathbf{M}\mathbf{D}
$$
Alternatively, we could just measure this lol. Which would be difficult, as we can't provide the necessary controlled environment for this. We needs this parameter to estimate the smoothing parameter and for uncertainty estimates in

\subsection{Exponential Smoothing}
Linear inhomogeneous ODE are of the form
$$
\frac{\dl y}{\dl x} + p(x)y = f(x) 
$$
With this, we can associate the vectorfield
$$
\mathbf{G} = 
\begin{pmatrix}
	p(x)y - f(x) \\ 1
\end{pmatrix}
$$
In general, this vectorfield isn't conservative since
$$
\curl{\mathbf{G}} = \begin{pmatrix}
	0 \\ 0 \\ -p(x)
\end{pmatrix}
$$
However, by using the integrating factor $\mu(x) = e^{\int_{0}^x p(u)du}$, we do find that
$$
\curl{\mu(x)\mathbf{G}} = \begin{pmatrix}
	0 \\ 0 \\ 0
\end{pmatrix}
$$
Where we used the fact that $\diff{\mu(x)}{x} = p(x)\mu(x)$. Since this new vectorfield is conservative, there must exist a scalarfield such that
$$
\mu(x)\mathbf{G} = \grad{g}
$$
From this we first draw
$$
\diffp{g}{y} = \mu(x) \quad \Longrightarrow \quad g = \mu(x)y + C_x(x) \quad \Longrightarrow \quad \diffp{g}{x} = \diff{\mu(x)}{x}y + \mu(x)\diff{y}{x} + \diff{C_x(x)}{x}
$$
Using the identity above, the original ODE and the fact that $g$ is a constant $C$, we find
$$
\diffp{g}{x} = \mu(x)f(x) + \diff{C_x(x)}{x} \quad \Longrightarrow \quad C_x = -\int \mu(x)f(x)dx
$$
We therefore find
$$
y(x) = \frac{1}{\mu(x)}\left(g - C_x(x)\right) = \frac{1}{\mu(x)}\left(C + \int_0^x \mu(s)f(s)ds\right)
$$
We know look at
$$
\diff{y}{x} = \frac{1}{\tau(x)}\left(s(x) - y\right)
$$
Using our result we find that
$$
y(x) = \frac{1}{\mu(x)}\left(y_0 + \int_0^x \mu(u)\frac{s(u)}{\tau(u)}du\right) \quad \text{with} \quad \mu(x) = e^{\int_0^x \frac{1}{\tau(\zeta)}d\zeta}
$$
We have the following identity
$$
\frac{\mu(u)}{\mu(x)} = e^{\int_0^u \frac{1}{\tau(\zeta)}d\zeta - \int_0^x \frac{1}{\tau(\zeta)}d\zeta} = e^{\int_x^u \frac{1}{\tau(\zeta)}d\zeta} = e^{\int_0^{u-x} \frac{1}{\tau(\zeta - x)}d\zeta}
$$
Such that we can neatly write
$$
y(x) = \int_{0}^x \exp(-\int_u^{x} \frac{1}{\tau(\zeta)}d\zeta)\frac{s(u)}{\tau(u)}du + \frac{y_0}{\mu(x)}
$$
If we imagine $\tau$ as a constant we find
$$
y(t) = \frac{1}{\tau}\int_{t_0}^{t}e^{-\frac{t-u}{\tau}}s(u)du + y_0e^{-\frac{t - t_0}{\tau}}
$$
We now want to discretize this, for this we'll use the time stamps $t_k = k\Delta t$, for the constant $\tau$ we find that one time step is given by
$$
y(t_k) = y_k= \frac{1}{\tau}\int_{t_{k-1}}^{t_k}e^{-\frac{t_k-u}{\tau}}s(u)du + y_{k-1}e^{-\frac{\Delta t}{\tau}} \approx \frac{s_k}{\tau}\int_{t_{k-1}}^{t_k}e^{-\frac{t_k-u}{\tau}}du + y_{k-1}e^{-\frac{\Delta t}{\tau}}
$$
We justify this approximation by taking $\Delta$ very small, and therefore assuming that $s(u)$ is constant over the interval. For our empirical data, this is also justified, as we only have the one data $s(t_k)$ to work with. Now we just explicitely work out the integral to find
$$
y_k = s_ke^{-\frac{t_k}{\tau}}\left(e^{\frac{t_k}{\tau}} - e^{\frac{t_{k-1}}{\tau}}\right) + y_{k-1}e^{-\frac{\Delta t}{\tau}} = s_k\left(1 - e^{-\frac{\Delta t}{\tau}}\right) + y_{k-1}e^{-\frac{\Delta t}{\tau}}
$$
By defining the smoothing parameter
$$
\alpha = 1 - e^{-\frac{\Delta t}{\tau}}
$$
We find that
$$
y_k = \left(1-\alpha\right)y_{k-1} + \alpha s_k
$$
Which is the exact formula for exponential smoothing. Using this smoothing parameter, we can construct a matrix $M$ which represents the smoothing operations, which can then in turn be inverted to recover the input signal $s(t)$ from the response $y(t)$. Alternatively, we can find it based on
$$
s_k = \frac{y_{k} - (1-\alpha) y_{k-1}}{\alpha}
$$
We try a similar approach to the non-constant $\tau$ case
$$
y_k = \int_{t_{k-1}}^{t_k} \exp(-\int_u^{t_k} \frac{1}{\tau(\zeta)}d\zeta)\frac{s(u)}{\tau(u)}du + y_{k-1}e^{-\int_{t_{k-1}}^{t_k}\frac{1}{\tau(\zeta)}d\zeta}
$$
We can apply a similar logic that in the interval between $t_{k-1}$ and $t_k$ we only have the one singular data point to work. So we assume this value to be constant over the interval. This gives us
$$
y_k \approx \frac{s_k}{\tau_k}\int_{t_{k-1}}^{t_k}e^{-\frac{t_k - u}{\tau_k}}du + y_{k-1}e^{-\frac{\Delta t}{\tau_k}}
$$
We want to emphasis that the dummy variable $u$ in this first integral is contained within $\left[t_{k-1}, t_k\right]$. Therefore, the integral $\int_u^{t_k} \frac{1}{\tau(\zeta)}d\zeta$ is within the interval where we assume a constant $\tau_k$. We can already see that this is the exact same expression as before, expect for $\tau_k$ which is dependent on the interval we look at. We therefore define the interval specific smoothing parameter
$$
\alpha_k = 1 - e^{-\frac{\Delta t}{\tau_k}}
$$
Such that
$$
s_k = \frac{y_k - (1 - \alpha_k)y_{k-1}}{\alpha_k}
$$
Lastly, we linearize our original differential equation. Our temperature terms abide by $g(T) = T^4$, we taylor expand around $T_0$ this to first order we find that
$$
g(T) = T^4 \approx T_0^4 + 4T_0^3(T-T_0)
$$
We apply this to both temperatures around the same $T_0$ in our original ODE
\begin{equation}
\begin{split}
	\dot{T}_g &= \frac{A}{\rho_gV_gc_{P,g}}\left[a_g\sigma T^4_{\text{MRT}} - \epsilon_g\sigma T^4_g - h\left(T_g - T_a\right)\right]
	\\
	&\approx\frac{A}{\rho_gV_gc_{P,g}}\left[4a_g\sigma T_0^3(T_{\text{MRT}} - T_0) - 4\epsilon_g\sigma T_0^3(T_g - T_0) - h\left(T_g - T_a\right)\right]
	\\
	&=\frac{A}{\rho_gV_gc_{P,g}}\left[4\epsilon_g\sigma T_0^3(T_{\text{MRT}} - T_g) - h\left(T_g - T_a\right)\right]
	\\
	&=\frac{A}{\rho_gV_gc_{P,g}}\left[4\epsilon_g\sigma T_0^3T_{\text{MRT}}+hT_a-(4\epsilon_g\sigma T_0^3 + h)T_g\right]
	\\
	&=\frac{A(4\epsilon_g\sigma T_0^3 + h)}{\rho_gV_gc_{P,g}}\left[\frac{4\epsilon_g\sigma T_0^3T_{\text{MRT}}+hT_a}{4\epsilon_g\sigma T_0^3 + h}-T_g\right]
\end{split}
\end{equation}
By linearizing our system we indeed find a first order inhomogenuous ODE, as such we can approximate what our globe thermometer does internally as exponential smoothing. This gives us the mathematical justification for using the inverse of this smoothing operation to recover the signal (in this case true MRT). We note, however, that we choose to taylor expand our the same $T_0$ for simplicity sack. The true MRT and globe temperature can however be very different, meaning that $T_0$ might not be a good fit for both values. If we taylor expand around different values we find that
$$
\dot{T_g} = \frac{A(4\epsilon_g\sigma T_{0,g}^3 + h)}{\rho_gV_gc_{P,g}}\left[\frac{4\epsilon_g\sigma T_{0,\text{MRT}}^3 (T_{\text{MRT}} -T_{0,\text{MRT}}) + 4\epsilon_g\sigma T_{0,g}^4 + hT_a}{4\epsilon_g\sigma T_{0,g}^3 + h}-T_g\right]
$$
From this we draw that 
$$
\frac{1}{\tau} = \frac{A(4\epsilon_g\sigma T_{0,g}^3 + h)}{\rho_gV_gc_{P,g}}
$$
We therefore suggest that in the exponential smoothing, values around the globe temperature are better.
\end{document}
