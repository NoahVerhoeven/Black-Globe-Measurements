...
In the specific case of our project, environment factors like air temperature, wind-speed, solar-angle, ... will be ever changing. Therefore, analysis of how we reach our equilibrium is warranted. Using the grey globe thermometer, the change in heat over time is 
$$
\dot{Q} = A\left[a_g\sigma T^4_{\text{MRT}} - \epsilon_g\sigma T^4_g - h\left(T_g - T_a\right)\right]
$$ 
We'll start by only assuming $T_g$ is a time dependent quantity. Following basic thermodynamics, we find that
$$
\dot{Q} = C_V \dot{T}
$$
Performing separation of variables we find that
$$
\int_{T_i}^{T_f}\frac{dT_g}{a_g\sigma T^4_{\text{MRT}} - \epsilon_g\sigma T^4_g - h\left(T_g - T_a\right)} = \int_{t_i}^{t_f} \frac{A}{C_V}dt = \frac{A}{C_V}\Delta t
$$

\subsection{Simulations}
Maybe we could simulate this
\\

A quick simulation, with arbitrary but semi realistic values, seems to be in agreement with the stated. We should keep in mind that this is would essentially simulate the change in temperature of the globe itself. Our response time, however, depends on how fast the air inside the globe thermometer can adjust. We can easily account for this. Assuming the final globe temperature $T_g$ won't be, at any time during the process, radically different from the initial air temperature inside our globe thermometer, we are justified to use conduction as the dominant heat transfer mechanism. This can be easily added to the simulation.
