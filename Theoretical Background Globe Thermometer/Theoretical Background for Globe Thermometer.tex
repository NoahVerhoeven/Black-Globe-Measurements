...
\begin{figure}[H]
	...
	\caption{Illustrative simulation for both inner air and shell. The simulated conditions were the same as figure \ref{fig:first-shell-only-simulation}. The inner temperature closely follows the globe temperature. For this simulation, we choose $h_i = 7 \unit{\joule\per\second\per\meter\squared\per\kelvin}$, which is a very conservative estimate. Higher $h_i$ yield faster adaptions of the inner air, and thus realistically the inner air follows the globe temperature even more closely.}
...
\subsection{Recreating the true MRT function}
