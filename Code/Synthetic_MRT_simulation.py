...
axis[0].plot(sol.t / 60, np.array([grey_body_MRT_estimate(T, h(t), T_a, epsilon) for T, t in zip(inner_temp, sol.t)]), label="Estimated MRT", color="tomato", linestyle="--")
...
axis[0].plot(sol.t / 60, np.array([MRT(t) for t in sol.t]), label="True MRT", color="black")
axis[0].plot(sol.t / 60, np.array([grey_body_MRT_estimate(T, h(t), T_a, epsilon) for T, t in zip(inner_temp, sol.t)]), label="Estimated MRT", color="gray", linestyle="--")
axis[0].legend()


axis[1].plot(sol.t / 60, shell_temp, label="Globe Temperature", color="black")
axis[1].plot(sol.t / 60, inner_temp, label="Inner Temperature", color="gray", linestyle="--")
axis[1].legend()
