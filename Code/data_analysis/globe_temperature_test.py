...
start_time = pd.Timestamp(2026, 4, 8, 14, 16, 0)
end_time = pd.Timestamp(2026, 4, 8, 15, 45, 0)

for key in data_dict:
    if key == "Vlinder":
        df = data_dict[key]
        df["datetime"] = pd.to_datetime(df['Datum'] + ' ' + df['Tijd (UTC)'], format='%Y-%m-%d %H:%M:%S')

        df_filtered = df.iloc[np.where((df["datetime"] >= start_time) & (df["datetime"] <= end_time))]
        df_filtered.plot(x="datetime", y="Globe Temperatuur", ax=axis[0], label=key, alpha=0.75, lw=2)
        df_filtered.plot(x="datetime", y="Temperatuur", ax=axis[1], label=key, alpha=0.75, markersize=2, lw=2)   
    else:
        df = data_dict[key]
