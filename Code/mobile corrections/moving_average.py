...
for window_size in range(90, 200):
    A = moving_average_matrix(true_mrt, window_size, mode=mode)

    moving_average_over_true_mrt = A@true_mrt

    new_error = np.sum(np.square(moving_average_over_true_mrt[window_size:] - estimated_mrt[window_size:]))
